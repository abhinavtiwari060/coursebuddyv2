              ...
              {playlist.map((v, i) => {
                 ...
                 return (
                    <button 
                      ...
                      className={`w-full flex items-center gap-4 p-4 rounded-[1.25rem] transition text-left group border border-transparent ${
                        isCurrent ? 'bg-gradient-to-br from-blue-600 to-indigo-600 shadow-xl shadow-blue-600/20 border-white/10' : 'hover:bg-white/5 hover:border-white/5'
                      }`}
                    >
                       <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0 transition-transform group-hover:scale-105 shadow-sm ${
                          isCurrent ? 'bg-white/10 text-white' : 'bg-slate-800 text-slate-500'
                       }`}>
